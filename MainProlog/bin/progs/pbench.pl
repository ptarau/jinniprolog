...
fa1(_M, _K, _P):-for(_,1,1000),fail;true.
...
get_cpu_time(T) :- statistics(runtime, [T0|_]),T is T0//10.
...
get_iterations_(ProbeIterations, Action, Iterations) :-
	...
	T is T1 - T0, % write(testtime(ProbeIterations)=T),nl,
	(T < 100-> 	% too close to UNIX clock resolution
		ProbeIterations1 is 2 * ProbeIterations,
		get_iterations_(ProbeIterations1, Action, Iterations)
	; Max is 500,
		( T > Max*ProbeIterations ->
			Iterations is 1
		;
		  Iterations is (Max//T)*ProbeIterations
		)
	).
	
get_iterations(Action, Iterations) :-
	get_iterations_(1, Action, Iterations).
	%write(iterations(Iterations,for(Action))),
	%nl.
