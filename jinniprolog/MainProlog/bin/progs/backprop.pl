...
go0:-statistics(runtime,_),train,statistics(runtime,[_,T]),write(time=T),nl.
...
forward_pass(Row,Col):-
	...
	sum(Os,0.0,Sum),NSum is 0-Sum,exp(NSum,Exp),
        Out is 1.0/(1.0+Exp),
...
learn :- 
	for(Iter,0,32000),
     ...
     nl,write(last_iteration(Iter)),nl.
...
rtest:-random_init,for(_,1,100),random(100,R),write(R),nl,fail.


random_init:-bb_let(rseed,rseed,43).

random(Max,R):-
        val(rseed,rseed,X),!,
        Y is (X*17+131) mod 31183,
        % Y is (X*7+131) mod 331,
        bb_set(rseed,rseed,Y),
        R is Y mod Max.
